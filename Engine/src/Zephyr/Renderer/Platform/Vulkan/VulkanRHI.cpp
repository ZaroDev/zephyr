...
namespace Zephyr
{
        ...
        void *user_data)
    {
        ...
        {
            VK_WARN("{0} - {1}: {2}", callback_data->messageIdNumber, callback_data->pMessageIdName, callback_data->pMessage);
        }
        else if (message_severity & VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT)
        {
            VK_ERROR("{0} - {1}: {2}", callback_data->messageIdNumber, callback_data->pMessageIdName, callback_data->pMessage);
        }
        return VK_FALSE;
    }
    ...
    bool VulkanRHI::Init()
    {
        ...
        m_Model = ModelImporter::LoadModel("Assets/cube/cube.gltf");
        UploadModel(m_Model);
    ...
    void VulkanRHI::DrawGeometry(VkCommandBuffer cmd)
    {
        ...
        VkRenderingInfo renderInfo = Utils::RenderingInfo(m_DrawExtent, &colorAttachment, nullptr);
        ...
        for (u32 lod = 0; lod < c_MaxLODCount; lod++)
        {
            ...
            for (auto& mesh : m_Model->GetMeshesAtLOD(lod))
            {
                ...
                auto& buffer = vkModel->GetBuffer(lod, index);
                
                GPUPushDrawConstants pushConstants{};
                pushConstants.World = Mat4(1.0f);
                pushConstants.VertexBuffer = buffer.VertexBufferAddress;
                

                vkCmdPushConstants(cmd, m_GeometryPipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(GPUPushDrawConstants), &pushConstants);
                vkCmdBindIndexBuffer(cmd, buffer.IndexBuffer.Buffer, 0, VK_INDEX_TYPE_UINT32);
    ...
    void VulkanRHI::UploadModel(Ref<Model> model)
    {
        for (u32 lod = 0; lod < c_MaxLODCount; lod++)
        {
            int index = 0;
            for (auto& mesh : model->GetMeshesAtLOD(lod))
            {
                const size_t vertexBufferSize = mesh.GetVertexCount() * sizeof(Vertex);
                const size_t indexBufferSize = mesh.GetIndexCount() * sizeof(u32);
                ...
                memcpy((char*)data + vertexBufferSize, mesh.GetIndices().data(), indexBufferSize);
    ...
    bool VulkanRHI::InitVulkan()
    {
        ...
        auto instanceReturn = builder.set_app_name(Application::Get().Specification().Name.c_str())
                                     ...
                                     .set_debug_callback(debug_utils_messenger_callback)
...
#error "Platform surface not implemented"
    ...
    bool VulkanRHI::InitCommands()
    {
        ...
        VK_ASSERT(vkCreateCommandPool(m_Device, &commandPoolInfo, nullptr, &m_ImmCommandPool));
        VkCommandBufferAllocateInfo cmdAllocInfo = Utils::CommandBufferAllocateInfo(m_ImmCommandPool);
        VK_ASSERT(vkAllocateCommandBuffers(m_Device, &cmdAllocInfo, &m_ImmCommandBuffer));
    ...
    bool VulkanRHI::InitSyncStructures()
    {
        ...
        VK_ASSERT(vkCreateFence(m_Device, &fenceCreateInfo, nullptr, &m_ImmFence));
    ...
    {
        return InitComputePipeline() && InitTrianglePipeline() && InitMeshPipeline();
    }
    ...
    bool VulkanRHI::InitTrianglePipeline()
    {
        VkShaderModule triangleFragShader;
        if (!LoadShaderModule("Shaders/Vulkan/colored_triangle.frag.spv", m_Device, &triangleFragShader))
        {
            VK_ERROR("Failed to load shader module");
            CORE_ASSERT(false);
            return false;
        ...
        if (!LoadShaderModule("Shaders/Vulkan/colored_triangle.vert.spv", m_Device, &triangleVertShader))
        {
            VK_ERROR("Failed to load shader module");
            CORE_ASSERT(false);
            return false;
        ...
        VK_ASSERT(vkCreatePipelineLayout(m_Device, &pipelineLayoutInfo, nullptr, &m_TrianglePipelineLayout));
    ...
    bool VulkanRHI::InitMeshPipeline()
    {
        VkShaderModule meshFragShader;
        if (!LoadShaderModule("Shaders/Vulkan/colored_triangle_mesh.frag.spv", m_Device, &meshFragShader))
        {
            VK_ERROR("Failed to load shader module");
            CORE_ASSERT(false);
            return false;
        ...
        if (!LoadShaderModule("Shaders/Vulkan/colored_triangle_mesh.vert.spv", m_Device, &meshVertShader))
        {
            VK_ERROR("Failed to load shader module");
            CORE_ASSERT(false);
            return false;
        ...
        bufferRange.size = sizeof(GPUPushDrawConstants);
        ...
        VK_ASSERT(vkCreatePipelineLayout(m_Device, &pipelineLayoutInfo, nullptr, &m_GeometryPipelineLayout));

        VulkanPipelineBuilder pipelineBuilder;
        pipelineBuilder.PipelineLayout = m_GeometryPipelineLayout;
        ...
        m_GeometryPipeline = pipelineBuilder.Build(m_Device);
    ...
    AllocatedBuffer VulkanRHI::CreateBuffer(size_t allocSize, VkBufferUsageFlags usage, VmaMemoryUsage memoryUsage)
    {
        ...
        VK_ASSERT(vmaCreateBuffer(m_Allocator, &bufferInfo, &allocInfo, &newBuffer.Buffer, &newBuffer.Allocation, &newBuffer.Info));
    ...
    {
        VK_ASSERT(vkResetFences(m_Device, 1, &m_ImmFence));
        VK_ASSERT(vkResetCommandBuffer(m_ImmCommandBuffer, 0));

        VkCommandBuffer cmd = m_ImmCommandBuffer;

        VkCommandBufferBeginInfo beginInfo = Utils::CommandBufferBeginInfo(VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT);
        ...
        VkCommandBufferSubmitInfo cmdInfo = Utils::CommandBufferSubmitInfo(cmd);
        VkSubmitInfo2 submitInfo = Utils::SubmitInfo(&cmdInfo, nullptr, nullptr);

        VK_ASSERT(vkQueueSubmit2(m_GraphicsQueue, 1, &submitInfo, m_ImmFence));
        VK_ASSERT(vkWaitForFences(m_Device, 1, &m_ImmFence, true, 9999999999));
    }
}